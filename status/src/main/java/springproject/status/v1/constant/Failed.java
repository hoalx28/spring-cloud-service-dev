...
public enum Failed {
        ...
        FIND_BY_ID(
                        1003, "can not retrieve {resource} by id: try again later.", HttpStatus.INTERNAL_SERVER_ERROR),
        ...
        ILL_LEGAL_JWT_TOKEN(
                        1017, "ill legal token: token has been edited or not publish by us.", HttpStatus.UNAUTHORIZED),
        ...
        BAD_CREDENTIALS(1020, "bad credentials: username or password not match.", HttpStatus.UNAUTHORIZED),
        JWT_TOKEN_NOT_SUITABLE(
                        1021, "access token and refresh token are not suitable.", HttpStatus.CONFLICT),
        VERIFY_IDENTITY(1022, "can not verify identity: try again later.", HttpStatus.INTERNAL_SERVER_ERROR),
        JWT_TOKEN_BLOCKED(1023, "token has been recall: can not use this any more", HttpStatus.UNAUTHORIZED),
        ...
        REFRESH_JWT_TOKEN(1026, "can not refresh token: try again later.", HttpStatus.INTERNAL_SERVER_ERROR),
        RETRIEVE_PROFILE(
